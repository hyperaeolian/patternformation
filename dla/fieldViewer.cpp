...
int countNeighbors(VEC3F cell){
    int count = 0;
    if (field(cell.x,cell.y+1) > 0 || field(cell.x,cell.y+1) < xRes-1){
        count++;
    }
    return field( cell.x  ,cell.y+1) + field( cell.x-1,cell.y) +
           field( cell.x+1,cell.y  ) + field( cell.x,cell.y-1);
       //}
}
///////////////////////////////////////////////////////////////////////
...
void runEverytime()
{
  MERSENNE_TWISTER twister(2);
  bool particle_added = false;
  while(!particle_added){
    VEC3F walker = insertWalker();
    //Perform random walk
    while (true){
      int side = twister.randInt(3);
      cout << side << endl;
      switch (side){
      ...
      int neighbors = countNeighbors(walker);
      //If we fall off the grid, insert a new random walker
      if (walker.x > xRes-1 || walker.x < 1 || walker.y > yRes-1 || walker.y < 1) {
          particle_added = false;
          goto stop;
      }
      //Walker becomes stuck if neighbor is stuck
      if (field(walker.x,walker.y+1) > 0 || ){
          field(walker.x,walker.y) = stuck;
          particle_added = true;
          goto stop;
      }
      stop:
