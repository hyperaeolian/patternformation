...
FIELD_2D A(xRes, yRes);
FIELD_2D B(xRes, yRes);
FIELD_2D laplacian(xRes, yRes);
FIELD_2D height(xRes, yRes);
FIELD_2D heightOld(xRes, yRes);
FIELD_2D backup(xRes, yRes);
...
std::vector<int> mandel(FIELD_2D& f){
    ...
    for (int x = 0; x < xRes; x++){
      for (int y = 0; y < yRes; y++){
        ...
        vals.push_back(esc);
        f(x,y) = esc;// - (log(log(mag(z) + epsilon)) / log(2)); 
      }
    }
    return vals;
}
...
void runEverytime()
{
  ...
  float dt = 0.001;
  float C = escapes[itr];//drand48() * 0.5;
  float xLen = 1.0;
  float dx = xLen / float(xRes);
  height = field;

    for (int x = 1; x < xRes-1; x++) for (int y = 1; y < yRes-1; y++){
      float radius = 2.0*M_PI * sqrt((x-xRes/2)*(x-xRes/2) + (y-yRes/2)*(y-yRes/2)) / (xRes/2);
      float h = cos(M_PI/2.0 * log2(4.0/M_PI*radius));
      filter(x,y) = pow(h,2);
      if (radius >= M_PI/2 || radius <= M_PI/8) filter(x,y) = 0; 
      laplacian(x,y) = -0.05 * height(x,y) + height(x+1,y) + height(x-1,y) + height(x,y+1) + height(x,y-1);
    }

  backup = height;
  height = pow(dt,2) / pow(dx,2) * C * laplacian + -2.0 * height;
  height -= heightOld;
  heightOld = backup;

   realField *= filter;
   imaginaryField *= filter;
   field = fieldFFT.inverseFFT(realField, imaginaryField);
    field += height;
   field.normalize();
   itr++;
   usleep(100000);
