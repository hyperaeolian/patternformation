...
FIELD_2D noiseTile(xRes,yRes);
FIELD_2D filter(xRes,yRes);
...
void build_NoiseTile(FIELD_2D& f){
    ...
    for (int x = 1; x < xRes-1; x++) for (int y = 1; y < yRes-1; y++){
      float radius = M_PI * sqrt((x-xRes/2)*(x-xRes/2) + (y-yRes/2)*(y-yRes/2)) / (xRes/2);
      float h = cos(M_PI/2.0 * log2(4.0/M_PI*radius));
      filter(x,y) = pow(h,2);
      if (radius >= M_PI/2 || radius <= M_PI/8) filter(x,y) = 0; 
    }

   realField *= filter;
   imaginaryField *= filter;
   f = fieldFFT.inverseFFT(realField, imaginaryField);
   f.normalize();
}

float noise(float x, float y){
    float xLeft = x * (float)xRes;
    float xRight = xLeft + 1;
    float yDown = y * (float)yRes; 
    float yUp = yDown + 1;

    float xFraction = xLeft - floor(xLeft);
    float yFraction = yDown - floor(yDown);

    while (xLeft >= xRes)  xLeft  -= (float)xRes;
    while (xRight >= xRes) xRight -= (float)xRes;
    while (yDown >= yRes)  yDown  -= (float)yRes;
    while (yUp >= yRes)    yUp    -= (float)yRes;

    float c0 = noiseTile(xLeft, yDown);
    float c1 = noiseTile(xRight,yDown);
    float c2 = noiseTile(xLeft, yUp);
    float c3 = noiseTile(xRight,yUp);

    float x0 = yFraction * c2 + (1 - yFraction) * c0;
    float x1 = yFraction * c3 + (1 - yFraction) * c1;

    return xFraction * x1 + (1 - xFraction) * x0;
}
...
void runOnce()
{
    ...
    for (int x = 1; x < xRes-1; ++x){
      for (int y = 1; y < yRes-1; ++y){
        for (int z = 1; z < N; ++z){
            float scale = pow(2,-5.0) * pow(2.0,z);
            float coeff = pow(0.5,z);
            field(x,y) += coeff * noise(scale * ((float)x/(float)xRes), scale * ((float)y/(float)yRes));
        }
      }
    }
}
