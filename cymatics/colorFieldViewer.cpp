...
void runEverytime()
{
    ...
    MATRIX eigenvectors;
    VECTOR eigenvalues;
    A.eigensystem(eigenvalues,eigenvectors); // O(N^3)

    int index = 0;
    for (int y = 0; y < yRes; y++){
        for (int x = 0; x < xRes; x++, index++){ 
            if (eigenvectors(index,row) <= 0 || eigenvectors(index,row) <= 0 || eigenvectors(index,row) <= 0){
              field(x,y).r = eigenvectors(index,row);
              field(x,y).b = eigenvectors(index,row)/2.0;
            } else {
              field(x,y).b = eigenvectors(index,row);
              field(x,y).g = eigenvectors(index,row)/2.0;
            }
        }
    }
...
void runOnce()
{
    for (int y = 0; y < yRes; y++) for (int x = 0; x < xRes; x++){
        int i = x + y * yRes;
        if (x != 0)      A(i,i-1)    = -1;
        if (x != xRes-1) A(i,i+1)    = -1;
        if (y != 0)      A(i,i-xRes) = -1;
        if (y != yRes-1) A(i,i+xRes) = -1;
        A(i,i) = 3;
    }
